...
const ChallengeTemplate = () => {
    ...
    const [selectedTab, setSelectedTab] = useState<ChallengeTabs>(ChallengeTabs.DESCRIPTION)
            ...
            <Box pb="4" minHeight="100vh">
                ...
                >
                    <Box width="858px" mr={width <= MOBILE_BREAKPOINT ? 0 : 5}>
                        {
                            (selectedTab === ChallengeTabs.DESCRIPTION || width > MOBILE_BREAKPOINT) && (
                                <DescriptionTab/>
                            )
                        }
                        {
                            (selectedTab === ChallengeTabs.DELIVERABLES && width <= MOBILE_BREAKPOINT) && (
                                <DeliverablesList
                                    data={deliverablesMock}
                                />
                            )
                        }
                        {
                            (selectedTab === ChallengeTabs.RANKING && width <= MOBILE_BREAKPOINT) && (
                                <RankingList data={rankingMock}/>
                            )
                        }
                        {
